...
#include "FaceEventServiceOutEventTestHandler.hh"

// initialize static singleton pointer to zero
SmartFaceRecognition* SmartFaceRecognition::_smartFaceRecognition = 0;

// constructor
SmartFaceRecognition::SmartFaceRecognition()
{
	std::cout << "constructor of SmartFaceRecognition\n";
	...
	faceEventServiceOut = NULL;
	faceEventServiceOutEventTestHandler = nullptr; 
	faceQueryServiceAnsw = NULL;
	faceQueryServiceAnswInputTaskTrigger = NULL;
	faceQueryServiceAnswHandler = NULL;
	recognitionTask = NULL;
	recognitionTaskTrigger = NULL;
	//smartFaceRecognitionParams = NULL;
	...
	connections.component.name = "SmartFaceRecognition";
	...
	connections.faceEventServiceOut.serviceName = "FaceEventServiceOut";
	connections.faceEventServiceOut.roboticMiddleware = "ACE_SmartSoft";
	connections.faceQueryServiceAnsw.serviceName = "FaceQueryServiceAnsw";
	connections.faceQueryServiceAnsw.roboticMiddleware = "ACE_SmartSoft";
	...
	// initialize members of PlainOpcUaSmartFaceRecognitionExtension
	
	// initialize members of SmartFaceRecognitionROSExtension
	
}

void SmartFaceRecognition::addPortFactory(const std::string &name, SmartFaceRecognitionPortFactoryInterface *portFactory)
{
	portFactoryRegistry[name] = portFactory;
}

void SmartFaceRecognition::addExtension(SmartFaceRecognitionExtension *extension)
{
	componentExtensionRegistry[extension->getName()] = extension;
}

SmartACE::SmartComponent* SmartFaceRecognition::getComponentImpl()
{
	return dynamic_cast<SmartFaceRecognitionAcePortFactory*>(portFactoryRegistry["ACE_SmartSoft"])->getComponentImpl();
}
 ...
 */
void SmartFaceRecognition::setStartupFinished() {
	stateSlave->setWaitState("Alive");
	std::cout << "ComponentDefinition initialization/startup finished." << std::endl;
}


Smart::StatusCode SmartFaceRecognition::connectCameraServiceIn(const std::string &serverName, const std::string &serviceName) {
 ...
 */
Smart::StatusCode SmartFaceRecognition::connectAndStartAllServices() {
 ...
 */
void SmartFaceRecognition::startAllTasks() {
 ...
 */
void SmartFaceRecognition::startAllTimers() {
}


Smart::TaskTriggerSubject* SmartFaceRecognition::getInputTaskTriggerFromString(const std::string &client)
{
...
void SmartFaceRecognition::init(int argc, char *argv[])
{
	try {
		Smart::StatusCode status;
		...
		// initializations of PlainOpcUaSmartFaceRecognitionExtension
		
		// initializations of SmartFaceRecognitionROSExtension
		...
		SmartFaceRecognitionPortFactoryInterface *acePortFactory = portFactoryRegistry["ACE_SmartSoft"];
		if(acePortFactory == 0) {
		...
		SmartACE::SmartComponent *component = dynamic_cast<SmartFaceRecognitionAcePortFactory*>(acePortFactory)->getComponentImpl();
		
		std::cout << "ComponentDefinition SmartFaceRecognition is named " << connections.component.name << std::endl;
		...
		faceEventServiceOutEventTestHandler = std::make_shared<FaceEventServiceOutEventTestHandler>();
		...
		faceEventServiceOutEventTestHandler = std::make_shared<FaceEventServiceOutEventTestHandler>();
		faceEventServiceOut = portFactoryRegistry[connections.faceEventServiceOut.roboticMiddleware]->createFaceEventServiceOut(connections.faceEventServiceOut.serviceName, faceEventServiceOutEventTestHandler);
		faceQueryServiceAnsw = portFactoryRegistry[connections.faceQueryServiceAnsw.roboticMiddleware]->createFaceQueryServiceAnsw(connections.faceQueryServiceAnsw.serviceName);
		faceQueryServiceAnswInputTaskTrigger = new Smart::QueryServerTaskTrigger<CommObjectRecognitionObjects::CommObjectRecognitionId, CommObjectRecognitionObjects::CommObjectRecognitionObjectProperties>(faceQueryServiceAnsw);
		...
		faceQueryServiceAnswHandler = new FaceQueryServiceAnswHandler(faceQueryServiceAnsw);
...
void SmartFaceRecognition::fini()
{
	...
	delete faceEventServiceOut;
	delete faceQueryServiceAnsw;
	delete faceQueryServiceAnswInputTaskTrigger;
	// destroy event-test handlers (if needed)
	faceEventServiceOutEventTestHandler;
	
	// destroy request-handlers
	delete environmentQueryServiceAnswHandler;
	delete faceQueryServiceAnswHandler;
	...
	// destruction of PlainOpcUaSmartFaceRecognitionExtension
	
	// destruction of SmartFaceRecognitionROSExtension
	
}

void SmartFaceRecognition::loadParameter(int argc, char *argv[])
{
	/*
	...
	try
	{
		...
		} else if(parameter.searchFile("SmartFaceRecognition.ini", parameterfile)) {
			parameterFileFound = true;
			std::cout << "load SmartFaceRecognition.ini parameter file\n";
			parameter.addFile(parameterfile);
		} else {
			std::cout << "WARNING: SmartFaceRecognition.ini parameter file not found! (using default values or command line arguments)\n";
		}
		...
		// load parameters for server FaceEventServiceOut
		parameter.getString("FaceEventServiceOut", "serviceName", connections.faceEventServiceOut.serviceName);
		if(parameter.checkIfParameterExists("FaceEventServiceOut", "roboticMiddleware")) {
			parameter.getString("FaceEventServiceOut", "roboticMiddleware", connections.faceEventServiceOut.roboticMiddleware);
		}
		// load parameters for server FaceQueryServiceAnsw
		parameter.getString("FaceQueryServiceAnsw", "serviceName", connections.faceQueryServiceAnsw.serviceName);
		if(parameter.checkIfParameterExists("FaceQueryServiceAnsw", "roboticMiddleware")) {
			parameter.getString("FaceQueryServiceAnsw", "roboticMiddleware", connections.faceQueryServiceAnsw.roboticMiddleware);
		}
		...
		// load parameters for PlainOpcUaSmartFaceRecognitionExtension
		
		// load parameters for SmartFaceRecognitionROSExtension
