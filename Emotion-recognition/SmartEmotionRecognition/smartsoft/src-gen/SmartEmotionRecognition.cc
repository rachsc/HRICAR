...
#include "EmotionEventServiceOutEventTestHandler.hh"

// initialize static singleton pointer to zero
SmartEmotionRecognition* SmartEmotionRecognition::_smartEmotionRecognition = 0;

// constructor
SmartEmotionRecognition::SmartEmotionRecognition()
{
	std::cout << "constructor of SmartEmotionRecognition\n";
	...
	emotionEventServiceOut = NULL;
	emotionEventServiceOutEventTestHandler = nullptr; 
	emotionQueryServiceAnsw = NULL;
	emotionQueryServiceAnswInputTaskTrigger = NULL;
	emotionQueryServiceAnswHandler = NULL;
	...
	//smartEmotionRecognitionParams = NULL;
	...
	connections.component.name = "SmartEmotionRecognition";
	connections.component.initialComponentMode = "Neutral";
	connections.component.defaultScheduler = "DEFAULT";
	connections.component.useLogger = false;
	
	connections.emotionEventServiceOut.serviceName = "EmotionEventServiceOut";
	connections.emotionEventServiceOut.roboticMiddleware = "ACE_SmartSoft";
	connections.emotionQueryServiceAnsw.serviceName = "EmotionQueryServiceAnsw";
	connections.emotionQueryServiceAnsw.roboticMiddleware = "ACE_SmartSoft";
	...
	// initialize members of PlainOpcUaSmartEmotionRecognitionExtension
	
	// initialize members of SmartEmotionRecognitionROSExtension
	
}

void SmartEmotionRecognition::addPortFactory(const std::string &name, SmartEmotionRecognitionPortFactoryInterface *portFactory)
{
	portFactoryRegistry[name] = portFactory;
}

void SmartEmotionRecognition::addExtension(SmartEmotionRecognitionExtension *extension)
{
	componentExtensionRegistry[extension->getName()] = extension;
}

SmartACE::SmartComponent* SmartEmotionRecognition::getComponentImpl()
{
	return dynamic_cast<SmartEmotionRecognitionAcePortFactory*>(portFactoryRegistry["ACE_SmartSoft"])->getComponentImpl();
}
 ...
 */
void SmartEmotionRecognition::setStartupFinished() {
	stateSlave->setWaitState("Alive");
	std::cout << "ComponentDefinition initialization/startup finished." << std::endl;
}


Smart::StatusCode SmartEmotionRecognition::connectCameraServiceIn(const std::string &serverName, const std::string &serviceName) {
 ...
 */
Smart::StatusCode SmartEmotionRecognition::connectAndStartAllServices() {
 ...
 */
void SmartEmotionRecognition::startAllTasks() {
 ...
 */
void SmartEmotionRecognition::startAllTimers() {
}


Smart::TaskTriggerSubject* SmartEmotionRecognition::getInputTaskTriggerFromString(const std::string &client)
{
...
void SmartEmotionRecognition::init(int argc, char *argv[])
{
	try {
		Smart::StatusCode status;
		...
		// initializations of PlainOpcUaSmartEmotionRecognitionExtension
		
		// initializations of SmartEmotionRecognitionROSExtension
		...
		SmartEmotionRecognitionPortFactoryInterface *acePortFactory = portFactoryRegistry["ACE_SmartSoft"];
		if(acePortFactory == 0) {
		...
		SmartACE::SmartComponent *component = dynamic_cast<SmartEmotionRecognitionAcePortFactory*>(acePortFactory)->getComponentImpl();
		
		std::cout << "ComponentDefinition SmartEmotionRecognition is named " << connections.component.name << std::endl;
		...
		emotionEventServiceOutEventTestHandler = std::make_shared<EmotionEventServiceOutEventTestHandler>();
		
		// create server ports
		// TODO: set minCycleTime from Ini-file
		emotionEventServiceOutEventTestHandler = std::make_shared<EmotionEventServiceOutEventTestHandler>();
		emotionEventServiceOut = portFactoryRegistry[connections.emotionEventServiceOut.roboticMiddleware]->createEmotionEventServiceOut(connections.emotionEventServiceOut.serviceName, emotionEventServiceOutEventTestHandler);
		emotionQueryServiceAnsw = portFactoryRegistry[connections.emotionQueryServiceAnsw.roboticMiddleware]->createEmotionQueryServiceAnsw(connections.emotionQueryServiceAnsw.serviceName);
		emotionQueryServiceAnswInputTaskTrigger = new Smart::QueryServerTaskTrigger<CommObjectRecognitionObjects::CommObjectRecognitionId, CommObjectRecognitionObjects::CommObjectRecognitionObjectProperties>(emotionQueryServiceAnsw);
		...
		emotionQueryServiceAnswHandler = new EmotionQueryServiceAnswHandler(emotionQueryServiceAnsw);
...
// run the component
void SmartEmotionRecognition::run()
{
...
void SmartEmotionRecognition::fini()
{
	...
	delete emotionEventServiceOut;
	delete emotionQueryServiceAnsw;
	delete emotionQueryServiceAnswInputTaskTrigger;
	delete environmentQueryServiceAnsw;
	delete environmentQueryServiceAnswInputTaskTrigger;
	// destroy event-test handlers (if needed)
	emotionEventServiceOutEventTestHandler;
	
	// destroy request-handlers
	delete emotionQueryServiceAnswHandler;
	...
	// destruction of PlainOpcUaSmartEmotionRecognitionExtension
	
	// destruction of SmartEmotionRecognitionROSExtension
	
}

void SmartEmotionRecognition::loadParameter(int argc, char *argv[])
{
	/*
	...
	try
	{
		...
		} else if(parameter.searchFile("SmartEmotionRecognition.ini", parameterfile)) {
			parameterFileFound = true;
			std::cout << "load SmartEmotionRecognition.ini parameter file\n";
			parameter.addFile(parameterfile);
		} else {
			std::cout << "WARNING: SmartEmotionRecognition.ini parameter file not found! (using default values or command line arguments)\n";
		}
		...
		// load parameters for server EmotionEventServiceOut
		parameter.getString("EmotionEventServiceOut", "serviceName", connections.emotionEventServiceOut.serviceName);
		if(parameter.checkIfParameterExists("EmotionEventServiceOut", "roboticMiddleware")) {
			parameter.getString("EmotionEventServiceOut", "roboticMiddleware", connections.emotionEventServiceOut.roboticMiddleware);
		}
		// load parameters for server EmotionQueryServiceAnsw
		parameter.getString("EmotionQueryServiceAnsw", "serviceName", connections.emotionQueryServiceAnsw.serviceName);
		if(parameter.checkIfParameterExists("EmotionQueryServiceAnsw", "roboticMiddleware")) {
			parameter.getString("EmotionQueryServiceAnsw", "roboticMiddleware", connections.emotionQueryServiceAnsw.roboticMiddleware);
		}
		...
		// load parameters for PlainOpcUaSmartEmotionRecognitionExtension
		
		// load parameters for SmartEmotionRecognitionROSExtension
